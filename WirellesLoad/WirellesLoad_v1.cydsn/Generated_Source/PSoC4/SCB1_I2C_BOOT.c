...
static uint8 SCB1_slReadBuf[SCB1_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* Reads from this buffer */
static uint8 SCB1_slWriteBuf[SCB1_I2C_BTLDR_SIZEOF_WRITE_BUFFER];

/* Flag to release buffer to be read */
static uint32 SCB1_applyBuffer;
...
static void SCB1_I2CResposeInsert(void);


/*******************************************************************************
* Function Name: SCB1_I2CCyBtldrCommStart
********************************************************************************
*
* Summary:
...
{
    SCB1_I2CSlaveInitWriteBuf(SCB1_slWriteBuf, SCB1_I2C_BTLDR_SIZEOF_WRITE_BUFFER);
    SCB1_I2CSlaveInitReadBuf (SCB1_slReadBuf, 0u);

    SCB1_SetCustomInterruptHandler(&SCB1_I2CResposeInsert);
    SCB1_applyBuffer = 0u;
...
/*******************************************************************************
* Function Name: SCB1_I2CCyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SCB1_I2CCyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SCB1_I2CCyBtldrCommRead
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  SCB1_slWriteBuf - used to store received command.
*  SCB1_slWrBufIndex - used to store current index within slave
*  write buffer.
*
*******************************************************************************/
cystatus SCB1_I2CCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        while (0u != timeoutMs)
        {
            ...
            if (0u != (SCB1_I2C_SSTAT_WR_CMPLT & SCB1_slStatus))
            {
                /* Copy number of written bytes */
                *count = (uint16) SCB1_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) SCB1_slWriteBuf,
                                              SCB1_BYTES_TO_COPY(*count, size));

                status = CYRET_SUCCESS;
                break;
            }

            CyDelay(SCB1_WAIT_1_MS);
            --timeoutMs;
        }
    }
...
/*******************************************************************************
* Function Name: SCB1_I2CCyBtldrCommWrite
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  SCB1_slReadBuf - used to store response.
*
*******************************************************************************/
cystatus SCB1_I2CCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;
        (void) memcpy((void *) SCB1_slReadBuf, (const void *) pData, (uint32) size);

        /* Read buffer is ready to be released to host */
        SCB1_applyBuffer = (uint32) size;
...
/*******************************************************************************
* Function Name: SCB1_I2CResposeInsert
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  SCB1_slRdBufSize - used to store slave read buffer size.
*  SCB1_slRdBufIndex - used to store the current index within the
*  slave read buffer.
*  SCB1_applyBuffer - the flag to release the buffer with
*  a response to read.
*
*******************************************************************************/
static void SCB1_I2CResposeInsert(void)
{
    if (SCB1_CHECK_INTR_SLAVE_MASKED(SCB1_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        if (SCB1_CHECK_I2C_STATUS(SCB1_I2C_STATUS_S_READ))
        {
            ...
            if (0u != SCB1_applyBuffer)
            {
                SCB1_slRdBufSize  = SCB1_applyBuffer;
                SCB1_slRdBufIndex = 0u;
                SCB1_applyBuffer  = 0u;
            }
        }
        else
        {
            ...
            if (SCB1_slRdBufIndex != SCB1_slRdBufSize)
            {
                SCB1_slRdBufIndex = SCB1_slRdBufSize;
            }
        }
    }
}
