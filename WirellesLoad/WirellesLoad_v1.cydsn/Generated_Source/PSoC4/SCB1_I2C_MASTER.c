...
/* Master variables */
volatile uint16 SCB1_mstrStatus;      /* Master Status byte  */
volatile uint8  SCB1_mstrControl;     /* Master Control byte */

/* Receive buffer variables */
volatile uint8 * SCB1_mstrRdBufPtr;   /* Pointer to Master Read buffer */
volatile uint32  SCB1_mstrRdBufSize;  /* Master Read buffer size       */
volatile uint32  SCB1_mstrRdBufIndex; /* Master Read buffer Index      */

/* Transmit buffer variables */
volatile uint8 * SCB1_mstrWrBufPtr;   /* Pointer to Master Write buffer */
volatile uint32  SCB1_mstrWrBufSize;  /* Master Write buffer size       */
volatile uint32  SCB1_mstrWrBufIndex; /* Master Write buffer Index      */
volatile uint32  SCB1_mstrWrBufIndexTmp; /* Master Write buffer Index Tmp */

#if (!SCB1_CY_SCBIP_V0 && \
    SCB1_I2C_MULTI_MASTER_SLAVE_CONST && SCB1_I2C_WAKE_ENABLE_CONST)
    static void SCB1_I2CMasterDisableEcAm(void);
#endif /* (!SCB1_CY_SCBIP_V0) */


/*******************************************************************************
* Function Name: SCB1_I2CMasterWriteBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  SCB1_mstrStatus  - used to store current status of I2C Master.
*  SCB1_state       - used to store current state of software FSM.
*  SCB1_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  SCB1_mstrWrBufPtr - used to store pointer to master write buffer.
*  SCB1_mstrWrBufIndex - used to current index within master write
*  buffer.
*  SCB1_mstrWrBufSize - used to store master write buffer size.
*
*******************************************************************************/
uint32 SCB1_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    errStatus = SCB1_I2C_MSTR_NOT_READY;

    if(NULL != wrData)  /* Check buffer pointer */
    {
        /* Check FSM state and bus before generating Start/ReStart condition */
        if(SCB1_CHECK_I2C_FSM_IDLE)
        {
            SCB1_DisableInt();  /* Lock from interruption */

            /* Check bus state */
            errStatus = SCB1_CHECK_I2C_STATUS(SCB1_I2C_STATUS_BUS_BUSY) ?
                            SCB1_I2C_MSTR_BUS_BUSY : SCB1_I2C_MSTR_NO_ERROR;
        }
        else if(SCB1_CHECK_I2C_FSM_HALT)
        {
            SCB1_mstrStatus &= (uint16) ~SCB1_I2C_MSTAT_XFER_HALT;
                              errStatus  = SCB1_I2C_MSTR_NO_ERROR;
        }
        else
        {
    ...
    if(SCB1_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
    #if (!SCB1_CY_SCBIP_V0 && \
        SCB1_I2C_MULTI_MASTER_SLAVE_CONST && SCB1_I2C_WAKE_ENABLE_CONST)
            SCB1_I2CMasterDisableEcAm();
    #endif /* (!SCB1_CY_SCBIP_V0) */

        /* Set up write transaction */
        SCB1_state = SCB1_I2C_FSM_MSTR_WR_ADDR;
        SCB1_mstrWrBufIndexTmp = 0u;
        SCB1_mstrWrBufIndex    = 0u;
        SCB1_mstrWrBufSize     = cnt;
        SCB1_mstrWrBufPtr      = (volatile uint8 *) wrData;
        SCB1_mstrControl       = (uint8) mode;

        slaveAddress = SCB1_GET_I2C_8BIT_ADDRESS(slaveAddress);

        SCB1_mstrStatus &= (uint16) ~SCB1_I2C_MSTAT_WR_CMPLT;
        ...
        if(SCB1_CHECK_I2C_MODE_RESTART(mode))
        {
            SCB1_I2C_MASTER_GENERATE_RESTART;
            SCB1_TX_FIFO_WR_REG = slaveAddress;
        }
        else
        {
            SCB1_TX_FIFO_WR_REG = slaveAddress;
            SCB1_I2C_MASTER_GENERATE_START;
        }
    }

    SCB1_EnableInt();   /* Release lock */
...
/*******************************************************************************
* Function Name: SCB1_I2CMasterReadBuf
********************************************************************************
*
* Summary:
...
*  SCB1_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  SCB1_mstrRdBufPtr - used to store pointer to master write buffer.
*  SCB1_mstrRdBufIndex - used to current index within master write
*  buffer.
*  SCB1_mstrRdBufSize - used to store master write buffer size.
*
*******************************************************************************/
uint32 SCB1_I2CMasterReadBuf(uint32 slaveAddress, uint8 * rdData, uint32 cnt, uint32 mode)
{
    ...
    if(SCB1_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        SCB1_state = SCB1_I2C_FSM_MSTR_RD_ADDR;
        SCB1_mstrRdBufIndex = 0u;
        SCB1_mstrRdBufSize  = cnt;
        SCB1_mstrRdBufPtr   = (volatile uint8 *) rdData;
        SCB1_mstrControl    = (uint8) mode;

        slaveAddress = (SCB1_GET_I2C_8BIT_ADDRESS(slaveAddress) | SCB1_I2C_READ_FLAG);

        SCB1_mstrStatus &= (uint16) ~SCB1_I2C_MSTAT_RD_CMPLT;
        ...
        /* Prepare reading */
        if(SCB1_mstrRdBufSize < SCB1_I2C_FIFO_SIZE)
        {
        ...
        else
        {
            ...
            SCB1_ENABLE_MASTER_AUTO_DATA_ACK;
...
/*******************************************************************************
* Function Name: SCB1_I2CMasterSendStart
********************************************************************************
*
* Summary:
...
uint32 SCB1_I2CMasterSendStart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(SCB1_CHECK_I2C_FSM_IDLE)
    {
        ...
        if(SCB1_CHECK_I2C_STATUS(SCB1_I2C_STATUS_BUS_BUSY))
        {
            errStatus = SCB1_I2C_MSTR_BUS_BUSY;
        }
        else
        {
            SCB1_DisableInt();  /* Lock from interruption */
            ...
            else /* Read direction */
            {
                SCB1_state = SCB1_I2C_FSM_MSTR_RD_DATA;
                         slaveAddress |= SCB1_I2C_READ_FLAG;
            }

            /* TX and RX FIFO have to be EMPTY */

            SCB1_TX_FIFO_WR_REG = slaveAddress; /* Put address in TX FIFO */
            SCB1_ClearMasterInterruptSource(SCB1_INTR_MASTER_ALL);

            SCB1_I2C_MASTER_GENERATE_START;


            while(!SCB1_CHECK_INTR_MASTER(SCB1_INTR_MASTER_I2C_ACK      |
                                                      SCB1_INTR_MASTER_I2C_NACK     |
                                                      SCB1_INTR_MASTER_I2C_ARB_LOST |
            ...
            if(SCB1_CHECK_INTR_MASTER(SCB1_INTR_MASTER_I2C_ACK))
            {
                errStatus = SCB1_I2C_MSTR_NO_ERROR;
            }
            else if(SCB1_CHECK_INTR_MASTER(SCB1_INTR_MASTER_I2C_NACK))
            {
                errStatus = SCB1_I2C_MSTR_ERR_LB_NAK;
            }
            else if(SCB1_CHECK_INTR_MASTER(SCB1_INTR_MASTER_I2C_ARB_LOST))
            {
                SCB1_state = SCB1_I2C_FSM_IDLE;
                             errStatus = SCB1_I2C_MSTR_ERR_ARB_LOST;
                             resetIp   = SCB1_I2C_RESET_ERROR;
            }
            else /* SCB1_INTR_MASTER_I2C_BUS_ERROR set is else condition */
            {
                SCB1_state = SCB1_I2C_FSM_IDLE;
                             errStatus = SCB1_I2C_MSTR_ERR_BUS_ERR;
                             resetIp   = SCB1_I2C_RESET_ERROR;
            }

            SCB1_ClearMasterInterruptSource(SCB1_INTR_MASTER_I2C_ACK      |
                                                        SCB1_INTR_MASTER_I2C_NACK     |
                                                        SCB1_INTR_MASTER_I2C_ARB_LOST |
                                                        SCB1_INTR_MASTER_I2C_BUS_ERROR);
...
/*******************************************************************************
* Function Name: SCB1_I2CMasterSendRestart
********************************************************************************
*
* Summary:
...
uint32 SCB1_I2CMasterSendRestart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(SCB1_CHECK_I2C_MASTER_ACTIVE)
    {
        slaveAddress = SCB1_GET_I2C_8BIT_ADDRESS(slaveAddress);
        ...
        */
        ...
        while(SCB1_CHECK_I2C_MASTER_CMD(SCB1_I2C_MASTER_CMD_M_START))
        {
...
/*******************************************************************************
* Function Name: SCB1_I2CMasterSendStop
********************************************************************************
*
* Summary:
...
uint32 SCB1_I2CMasterSendStop(void)
{
    ...
    if(SCB1_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        */
        ...
        while(!SCB1_CHECK_INTR_MASTER(SCB1_INTR_MASTER_I2C_STOP     |
                                                  SCB1_INTR_MASTER_I2C_ARB_LOST |
                                                  SCB1_INTR_MASTER_I2C_BUS_ERROR))
        {
        ...
        if(SCB1_CHECK_INTR_MASTER(SCB1_INTR_MASTER_I2C_STOP))
        {
            errStatus = SCB1_I2C_MSTR_NO_ERROR;
        }
        else if(SCB1_CHECK_INTR_MASTER(SCB1_INTR_MASTER_I2C_ARB_LOST))
        ...
        else /* SCB1_INTR_MASTER_I2C_BUS_ERROR is set */
        {
            errStatus = SCB1_I2C_MSTR_ERR_BUS_ERR;
            resetIp   = SCB1_I2C_RESET_ERROR;
        }
...
/*******************************************************************************
* Function Name: SCB1_I2CMasterWriteByte
********************************************************************************
*
* Summary:
...
uint32 SCB1_I2CMasterWriteByte(uint32 theByte)
{
    ...
    if(SCB1_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        else /* SCB1_INTR_MASTER_I2C_BUS_ERROR set is */
        {
            SCB1_state = SCB1_I2C_FSM_IDLE;
                         errStatus = SCB1_I2C_MSTR_ERR_BUS_ERR;
                         resetIp   = SCB1_I2C_RESET_ERROR;
...
/*******************************************************************************
* Function Name: SCB1_I2CMasterReadByte
********************************************************************************
*
* Summary:
...
uint32 SCB1_I2CMasterReadByte(uint32 ackNack)
{
    ...
    theByte = SCB1_I2C_INVALID_BYTE;

    /* Check FSM state before read byte */
    if(SCB1_CHECK_I2C_MASTER_ACTIVE)
    {
        while((!SCB1_CHECK_INTR_RX(SCB1_INTR_RX_NOT_EMPTY)) &&
              (!SCB1_CHECK_INTR_MASTER(SCB1_INTR_MASTER_I2C_ARB_LOST |
                                                  SCB1_INTR_MASTER_I2C_BUS_ERROR)))
        {
        ...
        if(SCB1_CHECK_INTR_RX(SCB1_INTR_RX_NOT_EMPTY))
        {
            theByte = SCB1_RX_FIFO_RD_REG;
...
/*******************************************************************************
* Function Name: SCB1_I2CMasterGetReadBufSize
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  SCB1_mstrRdBufIndex - used to current index within master read
*  buffer.
*
*******************************************************************************/
uint32 SCB1_I2CMasterGetReadBufSize(void)
{
    return(SCB1_mstrRdBufIndex);
}


/*******************************************************************************
* Function Name: SCB1_I2CMasterGetWriteBufSize
********************************************************************************
*
* Summary:
...
{
    return(SCB1_mstrWrBufIndex);
}


/*******************************************************************************
* Function Name: SCB1_I2CMasterClearReadBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SCB1_I2CMasterClearWriteBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SCB1_I2CMasterStatus
********************************************************************************
*
* Summary:
...
uint32 SCB1_I2CMasterStatus(void)
{
    ...
    status = (uint32) SCB1_mstrStatus;

    if (SCB1_CHECK_I2C_MASTER_ACTIVE)
    {
        /* Add status of master pending transaction: MSTAT_XFER_INP */
        status |= (uint32) SCB1_I2C_MSTAT_XFER_INP;
    }
...
/*******************************************************************************
* Function Name: SCB1_I2CMasterClearStatus
********************************************************************************
*
* Summary:
...
uint32 SCB1_I2CMasterClearStatus(void)
{
    ...
    status = (uint32) SCB1_mstrStatus;
    SCB1_mstrStatus = SCB1_I2C_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: SCB1_I2CReStartGeneration
********************************************************************************
*
* Summary:
...
void SCB1_I2CReStartGeneration(void)
{
    ...
    uint32 status = SCB1_I2C_STATUS_REG;

    while(SCB1_WAIT_SDA_SET_HIGH)
    {
        /* Wait when slave release SDA line: SCL tHIGH is complete */
    }

    /* Prepare DR register to drive SCL line */
    SCB1_SET_I2C_SCL_DR(SCB1_I2C_SCL_LOW);

    /* Switch HSIOM to GPIO: SCL goes low */
    SCB1_SET_I2C_SCL_HSIOM_SEL(SCB1_HSIOM_GPIO_SEL);

    /* Disable SCB block */
    SCB1_CTRL_REG &= (uint32) ~SCB1_CTRL_ENABLED;

    if(0u != (status & SCB1_I2C_STATUS_M_READ))
    {
        /* Generate NACK use GPIO functionality */
        SCB1_SET_I2C_SCL_DR(SCB1_I2C_SCL_LOW);
        CyDelayUs(SCB1_I2C_TLOW_TIME); /* Count tLOW */

        SCB1_SET_I2C_SCL_DR(SCB1_I2C_SCL_HIGH);
        while(SCB1_WAIT_SCL_SET_HIGH)
        {
            /* Wait until slave releases SCL in case if it stretches */
        }
        CyDelayUs(SCB1_I2C_THIGH_TIME); /* Count tHIGH */
    }
    ...
    SCB1_I2C_MASTER_CMD_REG = SCB1_I2C_MASTER_CMD_M_START;

    /* Enable SCB block */
    SCB1_CTRL_REG |= (uint32) SCB1_CTRL_ENABLED;

    /* Switch HSIOM to I2C: */
    SCB1_SET_I2C_SCL_HSIOM_SEL(SCB1_HSIOM_I2C_SEL);
    ...
    cmd  = SCB1_I2C_MASTER_CMD_M_START;
    cmd |= SCB1_CHECK_I2C_STATUS(SCB1_I2C_STATUS_M_READ) ?
                (SCB1_I2C_MASTER_CMD_M_NACK) : (0u);

    SCB1_I2C_MASTER_CMD_REG = cmd;
    ...
    SCB1_I2C_MULTI_MASTER_SLAVE_CONST && SCB1_I2C_WAKE_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: SCB1_I2CMasterDisableEcAm
    ********************************************************************************
    *
    * Summary:
    ...
    static void SCB1_I2CMasterDisableEcAm(void)
    {
        ...
        * This applicable only for Multi-Master-Slave with wakeup enabled. Ticket ID#192742 */
        if (0u != (SCB1_CTRL_REG & SCB1_CTRL_EC_AM_MODE))
        {
            ...
            SCB1_Stop();
            SCB1_CTRL_REG &= (uint32) ~SCB1_CTRL_EC_AM_MODE;
            SCB1_Enable();
        }
    }
#endif /* (!SCB1_CY_SCBIP_V0) */
